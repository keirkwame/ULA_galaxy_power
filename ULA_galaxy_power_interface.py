...
def execute(block, config):
    ...
    monopole = np.zeros((z_survey.shape[0], nk))
    quadrupole = np.zeros_like(monopole)

    for i, z in enumerate(z_survey):
        idx_distance = np.where(z_distance == z)[0][0]
        idx_growth = np.where(z_growth == z)[0][0]
        idx_power = np.where(z_power == z)[0][0]
        ...
        bird = pyb.Bird(k_power, pk[idx_power], f[idx_growth, 0], d_a[idx_distance],
        ...
        quadrupole[i, :] = bird.fullPs[1] + (4. * f[idx_growth, 0] * stochastic_term / 15.)

    #Save galaxy multipoles to block
    block.put_grids('EFT_of_LSS', 'k_multipoles', k, 'z_multipoles', z_survey, 'monopole', monopole, 'quadrupole',
                    quadrupole)
